...
- Le log est rendu en **SVG** ; les figurés sont des `<pattern>` SVG.
- Fonctions présentes : fiche de sondage, couches (de / à, lithologie, description, PID, indice organoleptique), échantillons, niveau d'eau, échelle réglable (px/m), impression A4, export / import JSON.
...
- Pas de `localStorage` / `sessionStorage` : la persistance passe par l'**export / import JSON**.
...
- Ne **jamais casser** l'impression A4 ni l'export / import JSON existants.
